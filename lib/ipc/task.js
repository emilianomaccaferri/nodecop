...
$("#create").on('click', (e) => {
  ...
  var taskName = $("#task-name").val().trim(),
      billingName = $("#task-billing-name").val().trim(),
      ...
      itemType = $(".item-picker.selected").attr('value'),
      ...
      paypalEmail = $("#task-paypal-email").val().trim(),
      paypalPassword = $("#task-paypal-password").val().trim(),
  ...
  if(!(taskName && billingName && billingEmail && billingTelephone && billingCity && billingAddress && billingZip && billingCountry &&
      itemType && itemName && itemColor && itemSize && ccType)){
  ...
  if(ccType === 'paypal'){
    ...
    item.request["credit_card[type]"] = "paypal"
    item.request["credit_card[month]"] = dummy
    item.request["credit_card[year]"] = new Date().getFullYear()
  }
  else{
    if(!(cnb && month && year && vval)){
      alert("Something is missing in your card infos")
      return;
    }
    item.request["credit_card[type]"] = ccType
    item.request["credit_card[cnb]"] = cnb
    item.request["credit_card[month]"] = month
    item.request["credit_card[year]"] = year
    item.request["credit_card[vval]"] = vval
  }
